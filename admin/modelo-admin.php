...
if ($_POST['registro'] == 'actualizar') {
    try {
        if(empty($_POST['password']) ) {
            ...
            $stmt = $conn->prepare('UPDATE admins SET usuario = ?, nombre = ?, nivel=?, password = ?, editado = NOW() WHERE id_admin = ? ');
            $stmt->bind_param("sssii", $usuario, $nombre, $nivel, $hash_password, $id_registro);
        }
        $stmt->execute();
